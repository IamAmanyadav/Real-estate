...
export default function PropertyCard({ property, index = 0, viewMode = "grid" }: PropertyCardProps) {
  ...
  // Live bidding pricing display
  const startingPrice = Number(property.reservePrice || property.price || 0);
  const currentPrice = property.currentHighestBid ? Number(property.currentHighestBid) : startingPrice;
  const hasBids = Boolean(property.currentHighestBid);
            ...
            <div className="flex flex-col sm:flex-row min-h-[170px]">
              ...
              <div className="relative sm:w-60 md:w-72 lg:w-80 shrink-0 aspect-[16/10] sm:aspect-auto overflow-hidden bg-muted">
                ...
                <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5">
                  ...
                  {property.isAuction && (
                    <Badge className="bg-amber-500 text-slate-950 border-0 rounded-full px-2 py-0.5 text-[10px] font-bold shadow-md flex items-center gap-1">
              ...
              <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-3 min-w-0">
                      ...
                      {property.isAuction ? (
                        <div className="space-y-0.5">
                          <div className="text-xs text-muted-foreground font-medium">
                            Starting: <span className="font-semibold text-foreground">{formatPrice(startingPrice)}</span>
                          </div>
                          <div className="text-lg sm:text-xl font-extrabold text-amber-500 font-mono">
                            {hasBids ? formatPrice(currentPrice) : "No Bids Yet"}
                          </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-border/60">
                  ...
                  <div className="flex items-center gap-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400 group-hover:translate-x-1 transition-transform">
                    {property.isAuction ? "View Live Auction" : "View Details"}
        ...
        <Card className="group h-full flex flex-col justify-between overflow-hidden border-border/60 hover:border-emerald-500/40 hover:shadow-xl hover:shadow-emerald-500/10 transition-all duration-300 py-0 gap-0 rounded-2xl bg-card">
          ...
          <div className="relative aspect-[16/10] overflow-hidden bg-muted shrink-0">
            ...
            <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center justify-between gap-2">
              {property.isAuction ? (
                <div className="space-y-0 text-white drop-shadow-md">
                  <div className="text-xs font-semibold text-amber-300">
                    {hasBids ? (
                      <span>Current: <strong className="text-sm font-bold text-white">{formatPrice(currentPrice)}</strong></span>
                    ) : (
                      <span>Start: <strong className="text-sm font-bold text-white">{formatPrice(startingPrice)}</strong></span>
                    )}
                  </div>
                </div>
              ) : (
            ...
            <div>
              ...
              {property.isAuction && (
                <div className="mt-2.5 p-2 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-between text-xs">
                  <div>
                    <span className="text-[10px] text-muted-foreground font-medium block">Starting Price</span>
                    <span className="font-semibold text-foreground font-mono">
                      {formatPrice(startingPrice)}
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] text-amber-500 font-bold block">Current Bid</span>
